...
<Warning>
  Stopping a session destroys the cloud machine. However, workspaces are automatically saved to the cloud every 30 seconds, so you can download them later with `catty download`.
</Warning>
...
Download a workspace snapshot from a session.

```bash
catty download <label> [path]
```
...
<Note>
  Workspaces are automatically saved to the cloud every 30 seconds. The first save happens about 10 seconds after session start.
</Note>
...
Sync a remote workspace to your current directory.

```bash
catty sync <label> [flags]
```
...
<Warning>
  This overwrites local files with the remote versions. Use `--dry-run` first to preview changes.
</Warning>
...
Manage secrets that are passed to remote sessions as environment variables.

```bash
catty secrets <subcommand>
```
...
```bash
catty secrets remove <name>
```

#### catty secrets test

Test a secret (currently supports `github`).
...
```bash
# Interactive GitHub token setup (recommended)
...
# Remove a secret
catty secrets remove MY_API_KEY
```

### Security

- Secrets are encrypted with AES-256-GCM
- Stored at `~/.catty/secrets.json` with `0600` permissions
- Encryption key is derived from machine-specific data (hostname + homedir)
- Secrets can only be decrypted on the machine they were created on

### GitHub Integration

With a GitHub token configured, Claude can:
- Clone private repositories
- Push commits
- Create pull requests using `gh` CLI

<Tip>
  Use `catty secrets add github` for a guided setup that opens the GitHub token creation page and validates your token.
</Tip>
