...
<Callout type="info">
  GitHub integration is built on [Secrets](/docs/concepts/secrets). The `catty secrets add github` command provides a guided setup specifically for GitHub tokens.
</Callout>
...
The token is stored encrypted at `~/.catty/secrets.json` using AES-256-GCM. It's only decrypted when starting a session and sent over HTTPS. See [Secrets](/docs/concepts/secrets) for encryption details.
...
- [Secrets](/docs/concepts/secrets) - How secrets work
- [catty secrets](/docs/cli/secrets) - CLI command reference
